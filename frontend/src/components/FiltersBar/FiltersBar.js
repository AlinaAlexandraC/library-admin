...
const FiltersBar = ({ selectedFilters, setSelectedFilters }) => {
    ...
    const typeArrowRef = useRef(null);
            ...
            <div className="filters-bar-wrapper">
                <ul className="filters-bar-options">
                    <li className="filter" onClick={() => {
                        toggleOptions("type");
                        rotateArrow(typeArrowRef, openFilter !== "type");
                    }}>
                        <div className="filter-name">Type</div>
                        <img src={arrowDownIconB} alt="arrow-down" className="arrow-down-icon-black" ref={typeArrowRef} />
                        <div className={`options-box ${openFilter === "type" ? "show" : ""}`}>
                            <ul>
                                {["Anime", "Book", "Manga", "Movie", "Series"].map((type, index) => (
                                    <li key={index} className={`option-${index}`}>
                                        <input type="checkbox" className="option-checkbox" checked={selectedFilters.type.includes(type)} onChange={() => handleCheckboxChange("type", type)} />
                                        <div>{type}</div>
                                    </li>
                                ))}
                            </ul>
                    ...
                    <li className="filter" onClick={() => {
                        ...
                        <div className={`options-box ${openFilter === "genre" ? "show" : ""}`}>
                            <ul>
                                {["Isekai", "Shonen", "Mecha", "Slice of Life", "Other"].map((genre, index) => (
                                    <li key={index} className={`option-${index}`}>
                ...
                <div className="filter-action-buttons">
                    <div className="filter-apply" onClick={applyFilters}>
